...
不会。桌面版会把后端数据库固定写到用户目录：

`%APPDATA%/PindouDesktop/data/pindou-db*`

因此你安装新版本（覆盖安装）后，历史数据会自动保留，不需要手工替换项目目录里的 `desktop/data`。

另外，新版本首次启动时会自动尝试把旧版常见位置（如安装目录下 `./data`）中的 `pindou-db*` 迁移到该目录；迁移成功后，后续升级将持续复用同一份数据。
